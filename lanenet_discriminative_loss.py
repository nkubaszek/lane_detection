...
def discriminative_loss_single(
    ...
    # Calculate pixel embedding mean vector
    ...
    # Calculate the regularization loss mentioned in the original Discriminative Loss paper
    l_reg = tf.reduce_mean(tf.norm(mu, axis=1, ord=1))

    # The combined loss is combined according to the parameters mentioned in the original Discriminative Loss paper
    param_scale = 1.
...
def discriminative_loss(prediction, correct_label, feature_dim, image_shape,
    ...
    """
    Iteratively calculate the loss according to the idea of the paper
