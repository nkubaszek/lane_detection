...
class LaneNetCluster(object):
    ...
    def _cluster(prediction, bandwidth):
        """
        Implement the cluster part of Section II of the paper
        :param prediction:
        :param bandwidth:
        :return:
        """
        ...
        try:
            ...
            log.error(err)
            return 0, [], []
        # log.info('Mean Shift takes time: {:.5f}s'.format(time.time() - tic))
        labels = ms.labels_
    ...
    def _get_lane_area(binary_seg_ret, instance_seg_ret):
        """
        Obtain the feature vectors of all lane lines on the instance segmentation map through
        the binary segmentation mask map
    ...
    def _thresh_coord(coord):
        """
        Filter the coordinate points of the lane line position of the instance, assuming that the lane line is continuous,
        so the coordinate transformation of the lane line point should be a smooth change and there should be no jumps
        :param coord: [(x, y)]
        :return:
        """
        pts_x = coord[:, 0]
    ...
    def _lane_fit(lane_pts):
        """
        Lane Line Polynomial Fittingparam lane_pts:
        :return:
        """
    ...
    def get_lane_mask(self, binary_seg_ret, instance_seg_ret):
        ...
        # If there are more than eight clusters, select the eight
        # clusters with the most samples in the class and keep them
        if num_clusters > 8:
            cluster_sample_nums = []
